...
//        Thread.sleep(2000);
   ...
   /* docobj.clickonGridIcon();
    ...
    @Story("story_id: U021 -Verify text on UI Audit Page.")
    @Description("Verify text present on UI Audit Page.")
    @Test(priority = 11, groups = "smoke", description = "Verify text on UI Audit Page")
    public void verifyUITextOnAuditPage() throws Exception {
        softAssert = new SoftAssert();
        auditPage = new AuditPage(driver);
        waitForloadSpinner();
        auditPage.clickOnauditTab();
        ...
        auditPage.getTheTextOfListOfAuditPage();
        Thread.sleep(3000);
        auditPage.clickOnFilterMatIcon();
        auditPage.getTheTextOfListOfFilterOnAuditPage();
        softAssert.assertEquals(driver.findElement(By.xpath("(//button[contains(@class,'mat-focus-indicator mt-1 text')])[2]")).getText(), "Clear");
        softAssert.assertEquals(driver.findElement(By.xpath("(//button[contains(@class,'mat-focus-indicator mat-flat-button mat-button')])[3]")).getText(), "Apply");
        Thread.sleep(3000);
        auditPage.getTheTextOfListOfButtonOnAuditPage();
