...
//    }
    ...
    @Description("Verify text present on UI reports Page.")
    @Test(priority = 21, groups = "smoke", description = "Verify text on UI reports Page")
    public void verifyUITextOnReportsPage() throws Exception {
        softAssert = new SoftAssert();
        waitForloadSpinner();
        reportPage = new ReportPage(driver);
        reportPage.clickOnReportTab();
        waitForloadSpinner();
        softAssert.assertEquals(driver.findElement(By.xpath("//span[@class='ml-2 primary-font-color']")).getText(), "Analytics");
        softAssert.assertEquals(driver.findElement(By.xpath("//button[contains(@class,\"mat-focus-indicator font-17 float-right mat\")]")).getText(), "Report ");
        reportPage.getTheTextOfListOfReportsPage();
        Thread.sleep(2000);
        reportPage.clickOnFilterMatIcon();
        reportPage.getTheTextOfListOfFilterOnReportsPage();
        Thread.sleep(2000);
